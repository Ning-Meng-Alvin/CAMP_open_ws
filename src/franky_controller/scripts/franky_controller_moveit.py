...
# Date: 2025-03-04 13:15:06
# LastEditors: Dianye dianye.huang@tum.de
# LastEditTime: 2025-08-20 12:03:02
# FilePath: /FrankaDemoAcquisition/src/record_demo/scripts/franka_control.py
# Description:
#     The franka gripper is controlled via Action, and the states are
#     streamed via ROS 1.0. The franka robotic arm is controlled & its
#     internal states are streamed via Franky. This script synthesizes these
#     two parts and publishes the necessary topics for visualization in RViz.

#     Update:
#     - Add one-shot joint goal subscriber (/franky/target_joint_position)
#     - Safely switch motion types: pause/zero velocity stream -> JointMotion -> resume velocity stream
# '''
...
#         self.ctrl_rate = rospy.get_param('~control_rate', 20.0)
...
#     def gripper_states_callback(self, msg: JointState):
#         self.gripper_width = msg.position[0]

#     def thread_pub_franka_states(self, robot: Robot):
#         """Publish states from the franka system (arm + ee pose)."""
...
#     def __init__(self):
#         # ROS
#         rospy.init_node('franky_ros', anonymous=False)
#         self.ros = FrankaROS()

#         # Franky
#         self.robot = Robot(self.ros.robot_ip)
#         self.robot.relative_dynamics_factor = RelativeDynamicsFactor(0.2, 0.3, 0.3)  # transl/rot/elbow
...
#         rospy.Subscriber("/spacenav/twist", twist_msg, self.spacenav_callback)
#         rospy.wait_for_message("/spacenav/twist", twist_msg)  # ensure first value
#         rospy.Subscriber("/franky/target_joint_position", JointState, self.on_joint_goal, queue_size=1)

#         print("Received first message: ", self.teleop_cmd)

#     # -------- inputs --------
#     def spacenav_callback(self, msg: twist_msg):
...
#         """Receive one-shot joint goal and cache it ."""
#         try:
#             if msg.name:
...
#             rospy.loginfo("[franky_ros] received one-shot joint goal (%d)", len(ordered))
...
#             )
#             # overwrite once with same motion type then join to flush
...
#     def cartesian_impedance_contrl(self, pose):
#         m_ci = CartesianImpedanceMotion(
#             Affine(), duration=1/30, target_type=ReferenceType.Relative
#         )
#         self.robot.move(m_ci, asynchronous=True)
...
#             if goal is not None:
#                 try:
#                     rospy.loginfo("[franky_ros] executing one-shot JointMotion...")
#                     # 1) gate off velocity stream & flush it
#                     self._vel_enabled = False
#                     self._stop_velocity_stream()
#                     # 2) switch motion type safely
#                     robot.move(JointMotion(goal))
#                     robot.join_motion()
#                     rospy.loginfo("[franky_ros] one-shot done, resume velocity servo.")
#                 except Exception as e:
#                     rospy.logerr("[franky_ros] one-shot failed: %s", e)
#                 finally:
#                     # 3) reopen velocity stream; reset last twist to avoid stale reuse
#                     self._last_twist = np.zeros(6, dtype=float)
#                     self._vel_enabled = True
